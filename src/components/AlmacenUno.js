...
const AlmacenUno = () => {
  ...
  function searchingTerm(term){
    ...
    const saveItems = async (valores)=>{
      if(item === ''){
        await db.collection('almacen1').doc().set(valores)
        ...
        await db.collection('almacen1').doc(item).update(valores)
    ...
    const EliminarProducto = async (id)=>{
      await db.collection('almacen1').doc(id).delete()
    ...
    const getProductos = ()=>{
        db.collection('almacen1').onSnapshot((querySnapshot)=>{
            const docs = []
    ...
    const getOne = async (id)=>{
      const doc = await db.collection('almacen1').doc(id).get()
      setItems({...doc.data()})
    }
